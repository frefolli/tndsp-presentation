...
\begin{frame}
  ...
  \begin{alertblock}{Rules}
    \begin{itemize}
      \item A solution is a set of unique Service Lines (w.r.t. symmetrical equality) forming a connected component covering the entire input network.
...
\begin{frame}
  ...
  \begin{block}{Definition}
    \begin{itemize}
      ...
      \item Each path is contained exaclty once (w.r.t. symmetrical equality) and has an associated ID.
...
\begin{frame}
  ...
  \begin{alertblock}{Individual's Feasibility Criteria}
    \begin{itemize}
      ...
      \item Each individual shall contain at most one occurrence of each row (w.r.t. symmetrical equality)
    \end{itemize}
  \end{alertblock}
  ...
  \begin{exampleblock}{Example of Individual}
    ...
    \begin{itemize}
      \item \textbf{$R_{0}$}: S1-S2-S4-S12
      \item \textbf{$R_{1}$}: S2-S3-S6-S15-S9
      \item \textbf{$R_{2}$}: S5-S4-S6-S15-S7-S10-S13
      \item \textbf{$R_{3}$}: S8-S15-S9
      \item \textbf{$R_{4}$}: S11-S13-S14
    \end{itemize}
  \end{exampleblock}
\end{frame}
  ...
  \begin{exampleblock}{Example of Frequency Setting and Individual Evaluation (see \ref{example:individual})}
    ...
    \begin{columns}
      ...
      \begin{column}{0.2\textwidth}
        \begin{itemize}
          \item \textbf{$f_{0}$}: 1.25
          \item \textbf{$f_{1}$}: 2.83
          \item \textbf{$f_{2}$}: 1.17
          \item \textbf{$f_{3}$}: 3.38
          \item \textbf{$f_{4}$}: 7.11
        \end{itemize}
      \end{column}
      \begin{column}{0.2\textwidth}
        \begin{itemize}
          \item \textbf{$F_{1}$}: 1004.4
          \item \textbf{$F_{2}$}: 1762.2
        \end{itemize}
      \end{column}
    \end{columns}
  \end{exampleblock}
